...
| GH-C97 | cli | Done/shipped: `todo add --note` persists through the add command and state writer and is projected on readback (#3323, closing #3477). Focused add/readback regression (including omit-note negative) lives in `examples/control_plane/todo-lifecycle-cli-smoke.py`; `todo-cli-smoke.py` also covers note persistence. | `python3 examples/control_plane/todo-lifecycle-cli-smoke.py` and `loopx check --scan-path docs/status-data-contract.md --scan-path docs/development/contributor-tasks.md` |
