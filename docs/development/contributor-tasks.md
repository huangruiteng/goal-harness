...
| GH-C70 | runtime | Claimed: a PR is open (#3664) adding a provider-neutral host-loop parity walkthrough (external scheduler worker + Pi) covering signed action selection, compact Turn receipts, independent validation, recoverable timeout/termination, replan, and terminal no-followup behavior. Review at exact head or add the missing negative it does not cover. | Focused fake-host smoke, `python3 -m pytest -q tests/test_loopx_turn_driver.py tests/test_external_scheduler_worker.py tests/test_pi_goal_mode.py`, and `loopx check --scan-path docs/reference/protocols/loopx-turn-v0.md --scan-path docs/integrations/runtime-connector-catalog.md --scan-path docs/development/contributor-tasks.md` |
