    ...
    specs: list[dict[str, Any]] = [
        {
            ...
            "instructions": [
                ...
                f"Prefer the event-driven scheduler (RFC Phase 5/6 new architecture) for task advancement when it is available. Each time you write or complete a business Todo, after `{cli_bin} refresh-state`, advance the queue by running `{cli_bin} codex-cli-local-scheduler-dispatch --goal-id <resolved-goal-id> --project . --completed-todo-id <just-written-or-completed-todo-id> --agent-id <resolved-agent-id> --event-driven` (add `--worker-id <resolved-agent-id>` to also claim the next ready task). This recomputes READY successors from handoff gates, records `task_ready`/`task_enqueued`/`task_dispatched` audit facts, and enqueues the next runnable task. When you need bounded batch advancement or a long-lived scheduling loop, prefer `{cli_bin} codex-cli-local-scheduler-resident --goal-id <resolved-goal-id> --project . --agent-id <resolved-agent-id> --iterations <n> --event-driven` (add `--execute-worker-command '<command>' --worker-command-prefix '<prefix>' --guard-checked` only to actually run the claimed task behind explicit gates). When a heartbeat poll is needed, prefer the merged path `{cli_bin} codex-cli-local-scheduler-merge --goal-id <resolved-goal-id> --project . --agent-id <resolved-agent-id> --merge` (records `heartbeat_observed`, runs `PolicyEngine` decision, and event-driven dispatch in one tick). These paths are opt-in: if any of these commands is unavailable or errors in this environment, fall back to the legacy `quota` / heartbeat flow and continue the business task without failing.",
                "To keep the new architecture active, ensure the relevant opt-in env flags are set when invoking the scheduler commands (they default on when `LOOPX_NEW_ARCHITECTURE` is unset): `LOOPX_EVENT_DRIVEN_DISPATCH=1` for dispatch/resident, `LOOPX_HEARTBEAT_EVENT_SOURCE=1` for the heartbeat event source, `LOOPX_MERGE_EVENT_DRIVEN_AND_HEARTBEAT=1` for the merged path, `LOOPX_USE_POLICY_ENGINE=1` so `quota should-run` attaches the unified `policy_decision`, and optionally `LOOPX_POLICY_DECISION_RECORD=1` to persist policy decisions as audit events.",
