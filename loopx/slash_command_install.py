    ...
    specs: list[dict[str, Any]] = [
        {
            ...
            "instructions": [
                ...
                f"If that packet exposes a goal-selection gate, rerun one exact choice before any mutation. For an argument-bearing task with no active agent interaction contract, treat this invocation as a new agent connection by default: choose a new public-safe agent id, preview `{cli_bin} register-agent --goal-id <selected-goal-id> --agent-id <new-agent-id> --require-new`, require `changed=true`, then apply it with `--execute` and rerun start-goal with explicit `--goal-id` and `--agent-id` before todo writeback. Only reuse an existing registered identity when the user explicitly asks to take over that exact agent's work; never infer takeover from a single registered agent or registry order.",
