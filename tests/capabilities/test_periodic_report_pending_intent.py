...
def test_consumption_preserves_frozen_capability_next_action_without_dropping_evidence(
    tmp_path: Path,
) -> None:
    registry, runtime = _fixture(tmp_path)
    sidecar = next(
    ...
    payload["intent"]["payload"]["project_progress"] = {
        ...
        "items": [
            {
                "item_id": "delivery",
                "title": "Finish the bounded analysis",
                "summary": "Bounded analysis finished",
                "content_kind": "outcome",
                "source_ref": "todo:todo_finished",
                "completed_at": "2026-08-30T09:00:00Z",
            },
            {
                "item_id": "next_action",
                "title": "Next action",
                "summary": "Resume network work",
                "content_kind": "next_action",
                "source_ref": "todo:todo_capacity",
            },
        ],
    }
    sidecar.write_text(json.dumps(payload), encoding="utf-8")
    ...
    facts = request["facts"]
    assert any(fact.get("source_ref") == "todo:todo_capacity" for fact in facts)


def test_consumption_fallback_preserves_observed_capabilities(tmp_path: Path) -> None:
    registry, runtime = _fixture(tmp_path)
...
- [ ] Resume the follow-up once network capacity returns.
  <!-- loopx:todo todo_id=todo_capacity status=open task_class=advancement_task claimed_by=report-agent action_kind=gated_work resume_when=capacity_available:network -->
""",
        encoding="utf-8",
    )
    sidecar = next(
    ...
    payload = json.loads(sidecar.read_text(encoding="utf-8"))
    payload["intent"]["payload"].pop("project_progress", None)
    payload["intent"]["payload"]["available_capabilities"] = ["network"]
    sidecar.write_text(json.dumps(payload), encoding="utf-8")
...
def test_consumption_fallback_excludes_capability_gated_todo_without_evidence(
    tmp_path: Path,
) -> None:
    registry, runtime = _fixture(tmp_path)
    state = registry.parent / "ACTIVE_GOAL_STATE.md"
...
""",
    ...
    payload = json.loads(sidecar.read_text(encoding="utf-8"))
    payload["intent"]["payload"].pop("project_progress", None)
    payload["intent"]["payload"].pop("available_capabilities", None)
    sidecar.write_text(json.dumps(payload), encoding="utf-8")
    ...
    facts = request["facts"]
    assert not any(fact.get("source_ref") == "todo:todo_capacity" for fact in facts)
